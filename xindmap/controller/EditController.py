...
class EditController:
    ...
    def on_editable_holder_editable_set(self, editable_holder, event):
        logging.debug(
            f"edit controller {id(self)}: on_editable_holder_editable_set(event={event})"
        )
    ...
    def on_input_stack_input_pushed(self, input_stack, event):
        logging.debug(
            f"edit controller {id(self)}: on_input_stack_input_pushed(event={event})"
        )
        ...
        if (
            input.type == xindmap.input.InputType.backspace
            and self.__editable is not None
        ):
    ...
    def on_state_holder_state_set(self, state_hoder, event):
        logging.debug(
            f"edit controller {id(self)}: on_state_holder_state_set(event={event})"
        )
